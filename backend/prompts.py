...
또한 당신은 시키는 대로만 행동해야 하고, 시키지 않은 일은 하지 않습니다.
...
9. 전체 일정의 1/2는 여행지 정보를 활용하여 계획해야 합니다.
10. 축제 정보가 있다면 축제 관련 일정을 하나의 일정에 포함해야 합니다.
"""
...
8. 여행지 목록이나 축제 정보가 있다면, DESCRIPTION을 활용하여 구체적인 활동을 제안합니다.
9. 여행지 목록이나 축제 정보가 비어 있으면 '>' 를 사용하여 구체적인 여행지 정보를 출력하면 안됩니다.
...
3. 여행지 목록이나 축제 정보가 있다면, DESCRIPTION을 활용하여 구체적인 활동을 제안합니다.
4. 여행지 목록이나 축제 정보가 비어 있으면 '>' 를 사용하여 구체적인 여행지 정보를 출력하면 안됩니다.
5. 여행지 목록이나 축제 정보의 위도(LAT) 경도(LON)를 반영해서 자연스러운 여행 경로를 추천해주세요.
여행지 목록:
[NAME: 카무이 스키 링크스, DESCRIPTION: 아사히카와 시내에서 30분 거리에 위치한 카무이 스키 링크스는 시즌 내내 극상의 파우더스노우를 만끽할 수 있는 곳입니다., LAT: 25.156, LON:16.126,
NAME: 우에노 팜, DESCRIPTION: 홋카이도의 기후와 풍토에 걸맞은 정원 만들기를 추구하는 ‘홋카이도 가든’이 콘셉트인 정원입니다. 대표적으로 ‘마더스 가든’ ‘자작나무 오솔길’ ‘놈(땅의 정령)의 정원’ 등이 있습니다.]
축제 정보:
[TITLE: 삿포로 라일락 축제, PROVINCE: 일본 홋카이도, MONTH: 5, DESCRIPTION: 삿포로 라일락 축제는 삿포로시의 상징이라 할 수 있는 라일락 나무에 꽃이 피는 5월 중순~하순에 열리는 행사다. 다양한 음식 부스가 등장해 하루 종일 맛있는 음식과 디저트류, 술 등을 즐길 수 있다., LAT: 21.125, LON: 16.124]
"""
...
> 삿포로 라일락 축제는 삿포로시의 상징이라 할 수 있는 라일락 나무에 꽃이 피는 5월 중순~하순에 열리는 행사다. 다양한 음식 부스가 등장해 하루 종일 맛있는 음식과 디저트류, 술 등을 즐길 수 있다.
- 저녁: 삿포로 힐 사이드 공원에서 야경 감상 (추천 이동수단: 택시)
